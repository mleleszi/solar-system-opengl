...
void resize_game(Game* game, int width, int height)
{
    game->width = width;
    game->height = height;
    game->left_pad.size = 150;
    game->right_pad.size = 150;
    restart_game(game);
}
...
void update_game(Game* game, double time)
{
    ...
    if (game->ball.x - game->ball.radius < 50 && game->ball.y - game->ball.radius < game->left_pad.position + game->left_pad.size &&
        game->ball.y - game->ball.radius > game->left_pad.position) {
        game->ball.x = game->ball.radius + 50;
        game->ball.speed_x *= -1;
    }
    if (game->ball.x + game->ball.radius > game->width - 50 && game->ball.y - game->ball.radius < game->right_pad.position + game->right_pad.size &&
        game->ball.y - game->ball.radius > game->right_pad.position) {
